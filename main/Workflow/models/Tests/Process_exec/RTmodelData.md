a1:Place {
    Name="a";
	Process = "d_out";
    Sync = False;
	Start = True;
}
a2:Place {
    Name="b";
	Process = "d_through";
    Sync = False;
	Start = False;
}
a3:Place {
    Name="c";
	Process = "d_print";
    Sync = False;
	Start = False;
}
a1flow_out1:Port {
	Name = "out";
}
a1data_out1:Port {
	Name = "id";
}
a2flow_in1:Port {
	Name = "in";
}
a2flow_out1:Port {
	Name = "out";
}
a2data_in1:Port {
	Name = "id";
}
a2data_out1:Port {
	Name = "id";
}
a3flow_in1:Port {
	Name = "in";
}
a3data_in1:Port {
	Name = "id";
}
rt_state:RTState {
}
d1data_out1:Port_data {
	Value = "None";
}
d2data_out1:Port_data {
	Value = "None";
}

:Transition (a1flow_out1 -> a2flow_in1) {
}
:Transition (a2flow_out1 -> a3flow_in1) {
}
:Data_flow (a1data_out1 -> a2data_in1) {
}
:Data_flow (a2data_out1 -> a3data_in1) {
}
:Port_flow_in (a2 -> a2flow_in1) {
}
:Port_flow_in (a3 -> a3flow_in1) {
}
:Port_flow_out (a1 -> a1flow_out1) {
}
:Port_flow_out (a2 -> a2flow_out1) {
}
:Port_data_in (a2 -> a2data_in1) {
}
:Port_data_in (a3 -> a3data_in1) {
}
:Port_data_out (a1 -> a1data_out1) {
}
:Port_data_out (a2 -> a2data_out1) {
}
:Running (rt_state -> a1) {
	Id="";
}
:RT_data_connect (rt_state -> d1data_out1) {
}
:RT_data_connect (rt_state -> d2data_out1) {
}
:Port_data_connect (d1data_out1 -> a1data_out1) {
}
:Port_data_connect (d2data_out1 -> a2data_out1) {
}